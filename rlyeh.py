...
move_dict = {sf.Keyboard.W:sf.Vector2(0, -5), sf.Keyboard.A:sf.Vector2(-5, 0), sf.Keyboard.S:sf.Vector2(0, 5), sf.Keyboard.D:sf.Vector2(5, 0)}
...
while w.is_open:
    for e in w.events:
        if type(e) is sf.KeyEvent and e.pressed:
            if e.code is sf.Keyboard.SPACE:
                if attacking and attacking_clock.elapsed_time.seconds >= 10:
                    attacking = False
                elif not attacking:
                    attacking = True
    ...
    if spawn_clock.elapsed_time.seconds >= .10 and not cth_dead:
        if len(enemies_list) > 10:
    ...
    for i in enemies_list:
        if collision.collides(i.global_bounds, player.global_bounds) and refresh_clock.elapsed_time.seconds > 1.5:
            hp -= 10
